...
from ListSerializersApp.serializers import (BuyBooksListSerializer)
...
class ListSerializersApiView(CreateModelMixin,RetrieveModelMixin,ListModelMixin,UpdateModelMixin,DestroyModelMixin,GenericViewSet):
    queryset=BookStore.objects.using("default").all()
    serializer_class=BuyBooksListSerializer
    # filter_fields = ["name"]
        ...
        try:
            ...
            for book in data.pop("books"):
                if BookStore.objects.filter(books=book).exists():
                    return Response("This book Id is Already Register Beacause this Book Id make OneToOne relationship")
                data["books"]=book
                ...
                if serializers.is_valid(raise_exception=True):
                    serializers.save()
            return Response(f"Successfully buy Books")
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if not Group.objects.using("default").filter(user=user,name="Admin").exists():
            if not self.queryset.using("default").filter(id=kwargs.get("pk"),user_id=user).exists():
    ...
    def partial_update(self, request, *args, **kwargs):
        try:
            ...
            datas=[]
            datas.append(data)
            serializers=self.serializer_class(instance,data=datas,partial=True)
            if serializers.is_valid(raise_exception=True):
                serializers.save()
                return Response("Data Updated Succesfully")
            print(serializers.errors,"dgdhrd")
        except Exception as e:
            return Response(str(e),status=status.HTTP_400_BAD_REQUEST)